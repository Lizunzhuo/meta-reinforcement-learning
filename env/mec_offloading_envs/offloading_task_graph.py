...
class OffloadingTask(object):
    ...
    def print_task(self):
        print("task id name:{}，task type name:{},task processing data size:{},task transmission_data_size:{}"
              .format(self.id_name,self.type_name,self.processing_data_size,self.transmission_data_size))

# Task_list = OffloadingTask(1,555,555,2)
# Task_list.print_task()

class OffloadingDotParser(object):
    """
    Psrser for Task_graphs
    Args:
    ...
    def __init__(self,file_name):
        ...
        self._calculate_depth_and_teansimisson_datasize()

    def _parser_task(self):
        jobs = self.dot_ob.get_name()
    ...
    def _parse_dependecies(self):
        ...
        dependency_matrix = np.zeros(shape=(task_number,task_number),dtype=np.float32)
        ...
        def calculate_depth_value(id):
            if id in ids_to_depth.keys():
                ...
                if len(self.pre_task_for_ids[id]) != 0:
                    depth = 1 + max([calculate_depth_value(pre_task_id) for pre_task_id in self.pre_task_for_ids[id]])
                else:
                    depth = 0
            return ids_to_depth[id]
...
class OffloadingTaskGraph(object):
    ...
    def _parse_from_dot(self,file_name):
        ...
        for pair in dependencies:
            ...
            self.min_data_size = np.max(self.dependency[self.dependency > 0.01])
    ...
    def encode_point_sequence_with_cost(self,resource_cluster):
        ...
        for i in range(self.task_number):
            ...
            down_link_cost = resource_cluster.down_transmission_cost(task.transmission_data_size)

            task_embeding_vector = [i,local_process_cost,up_link_cost,mec_process_cost,down_link_cost]
            ...
            for succs_task_index in range(i+1,self.task_number):
                if self.dependency[succs_task_index][i] > 0.1:
                    succs_task_index_set.append(succs_task_index)
            while(len(succs_task_index_set) < 6):
                succs_task_index_set.append(-1.0)
    ...
    def encode_point_sequence_with_rankin_and_cost(self,sorted_task,resource_cluster):
    ...
    def encode_edge_sequence(self):
        ...
        for i in range(0,len(self.edge_set)):
            if i < len(self.edge_set):
                ...
                edge_array.append([0,0,0,0,0,0,])

        # input edge sequence refers to stare node index
    ...
    def print_graphic(self):
        ...
        print("This is pre_task_sets:",self.pre_task_sets)
        print("This is edge set:",self.edge_set)

    def prioritize_task(self,resource_cluster):
